<script>
		if(localStorage.getItem("cTicket_awb") !== null){
			$('#resi').val(localStorage.cTicket_awb);
		}	
			...
			$.post( site_url + 'app/check_ticket',{jenis_kiriman : jenis_kiriman, resi:$('#resi_manual').val()},function(res){
				if(res.new_ticket){
					if(res.xray_ticket){
						...
						var notes = "";
						if(localStorage.getItem("cTicket_notes") !== null){
							notes += localStorage.cTicket_notes;
							notes += "\n";
						}
						notes += res.tracking_ticket;
						$('#note').val(notes);
					}
        ...
        $('#form-helper-btn-check').on('click', function(){
			...
			$.post ( site_url + 'app/check_ticket', {resi:$('#resi').val()}, function(res){
				$('#jenis_layanan_txt_info').text(res.jenis_layanan_txt);
				$('#jenis_layanan').val(res.jenis_layanan).trigger('change');
				if(res.new_ticket){
	...
	$('#tujuan_pengaduan').select2({
        ...
        ajax: {
            url: site_url + 'app/get_kantor_pos_tujuan',
            dataType: 'json',
            method:"POST",
            delay: 250,
...
<section>
    <!--<form id="form-tracking-resi">-->
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<select name="jenis_layanan" id="jenis_layanan" class="form-control select2min required" style="width:100%">
							...
							<option value="PKH"> Paket Kilat Khusus </option>
							<option value="SKH"> Surat Kilat Khusus </option>
							<option value="PPB"> Paket Pos Biasa </option>
							<option value="PJE"> Paket Jumbo Ekonomi </option>
							<option value="PJM"> Paket Jumbo Motor </option>
							<option value="CRU"> Cargo Ritel Udara </option>
							<option value="CRD"> Cargo Ritel Darat </option>
							<option value="PJB"> Paket Jumbo Barang </option>
						</select>
						<small id="jenis_layanan_txt_info"></small>
					</div>
				</div>
			</div>
			...
			<div class="row">
				...
				<div class="col-md-4 internationalonly hidden">
					...
					<select name="jenis_layanan_intl" id="jenis_layanan_intl" class="form-control select2min" style="width:100%">
						...
						<option value="PCU"> Paket Cepat Inter Udara</option>
		...
		<span class="btn btn-success fileinput-button">
			<span><i class="fa fa-upload"></i> Tambah Dokumen</span>
